...
The function `compute_F_matrix(...)` below is proposed to fill in a 2D matrix, 
also known as `F` matrix. 
The row names of this matrix are the characters of the 1st sequence A,
and the column names are the characters of the 2nd sequence B.
You need to complete the code to implement the scoring system for this purpose, 
which is determined by three parameters with their default values: 
`match=1.0, mismatch=-1.0, gap=-1.0`, 
where *match/mismatch* is equivalent to the score matrix and *gap* is the linear gap penalty.
The function will return the `F` matrix.
...
compute_F_matrix <- function(A, B, gap=-1.0, match=1.0, mismatch=-1.0) {
  ...
  # sequence A in row names, B in col names.
  rownames(F) <- c("*", unlist(strsplit(A, split = "")) )
  colnames(F) <- c("*", unlist(strsplit(B, split = "")) )
  ...
  # sequence X in row names of F, and Y in column names of F.
  # start scoring. 
  for (i in 2:nrow(F)) {
    for (j in 2:ncol(F)) {
...
Given sequence x and y, test your implementation in the following code block.
It will print `F`, if no uncompleted code.  
...
For example, if there is the multiple choice to move either vertical or diagonal, 
then the high road approach will take the vertical move. 
If either diagonal or horizontal, then take diagonal.
...
trace_back <- function(F, A, B, gap=-1.0, match=1.0, mismatch=-1.0) {
  ...
  while (i > 1 || j > 1) {
    ...
    # hint: create AlignmentA and AlignmentB
...
Remember to always take the high road during traceback (see 1.2). 

Note you may need to break ties to decide where the traceback will start, if there are multiple maximum scores in the matrix. This will be done by another rule: we will always take the maximum score in the rightmost position (largest column) and then lowest (largest row) in `F` to start the traceback. This is separate from the "high-road" rule for traceback.
...
local_alignment <- function(A, B, F, d=-2.0) {
  ...
  # To start the traceback, we will always take the maximum score 
  # in the rightmost position (largest column), and then lowest (largest row) in `F`, 
